...
const getSelectedName = (attr: any, activeVariation: VariationAttribute) => {
  if (attr?.terms?.nodes) {
    return attr.terms.nodes.find((node: { id: string }) => node.id === activeVariation.value)?.name;
  }
    ...
    <div v-for="(attr, i) in attributes" :key="i" class="flex flex-wrap py-2 relative justify-between">
        ...
        <div class="flex gap-2">
          <span v-for="(term, termIndex) in attr.terms.nodes" :key="termIndex">
            <Tooltip :text="term.name">
              <label :for="`${term.id}_${termIndex}`">
                <input
                  :id="`${term.id}_${termIndex}`"
                  :ref="attr.name"
                  class="hidden"
                  :checked="termIndex == 0"
                  type="radio"
                  ...
                  :value="term.id"
                  @change="updateAttrs" />
                <span class="color-button" :class="`color-${term.id}`" :title="`${attr.name}: ${term}`"></span>
              </label>
            </Tooltip>
          </span>
        </div>
      ...
      <div v-else-if="attr.terms.nodes && attr.terms.nodes?.length > 8" class="grid gap-2">
        ...
        <select :id="attr.name" :ref="attr.name" :name="attr.name" required class="border-white shadow" @change="updateAttrs">
          ...
          <option v-for="(term, dropdownIndex) in attr.terms.nodes" :key="dropdownIndex" :value="term.id" v-html="term.name" :selected="dropdownIndex == 0" />
        </select>
      </div>
          ...
          <span v-for="(term, index) in attr.terms.nodes" :key="index">
            <label :for="`${term.id}_${index}`">
              <input
                ...
                :value="term.id"
                @change="updateAttrs" />
              <span class="radio-button" :class="`picker-${term.id}`" :title="`${attr.name}: ${term.id}`">{{ term.name }}</span>
            </label>
          </span>
        </div>
      </div>
