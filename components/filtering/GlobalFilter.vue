...
const selectedTerms = ref(getFilter(attribute.id) || []);
const filterTitle = ref(attribute.label || attribute.id);
...
const checkboxChanged = () => {
  setFilter(attribute.id, selectedTerms.value);
};
</script>
...
<template>
  ...
  <div v-show="isOpen" class="mt-3 mr-1 max-h-[240px] grid gap-1 overflow-auto custom-scrollbar">
    <div v-for="term in attribute.terms" :key="term.id" class="flex gap-2 items-center">
      <input :id="term.id" v-model="selectedTerms" type="checkbox" :value="term.id" @change="checkboxChanged" />
      <label :for="term.id" class="cursor-pointer m-0 text-sm flex items-center flex-wrap">
