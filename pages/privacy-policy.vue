<template>
  <div class="relative font-italic overflow-hidden">
    ...
    <div class="container flex items-start gap-16 relative -mt-16 bg-white rounded-t-3xl sm:p-8 xs:p-4 shadow-lg">
      <div class="my-8 " v-if="!loading && privacyPolicy.data && privacyPolicy.data.length > 0">
        <p v-html="privacyPolicy.data[0].smalldescription[locale]"></p>
        <p v-html="privacyPolicy.data[0].description[locale]"></p>
      </div>
...
import { usePrivacyPolicy } from '~/stores/privacyPolicy';

const {locale} = useI18n()

const privacyPolicyStore = usePrivacyPolicy();
const privacyPolicy = computed(() => privacyPolicyStore.privacyPolicy);
const loading = computed(() => privacyPolicyStore.loading);

onBeforeMount(async() => {
  await privacyPolicyStore.getPrivacyPolicyInfo();
});
</script>
