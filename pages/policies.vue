<template>
  <div class="relative font-italic overflow-hidden">
    ...
    <div class="container flex items-start gap-16 relative -mt-16 bg-white rounded-t-3xl sm:p-8 xs:p-4 shadow-lg">
      <div class="my-8 w-full" v-if="!loading && policies.data && policies.data.length > 0">
        <div v-for="policy in policies.data" :key="policy.id" class="mb-12">
          <h2 class="text-2xl font-bold mb-4">{{ policy.title[locale] }}</h2>
          <div v-html="policy.description[locale]"></div>
        </div>
      </div>
...
const policiesStore = usePolicies();
const policies = computed(() => policiesStore.policies);
const loading = computed(() => policiesStore.loading);

onBeforeMount(async () => {
  await policiesStore.getPolicies();
});
</script>
